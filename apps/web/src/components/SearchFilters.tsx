...
interface SearchFiltersProps {
  ...
  sortBy: SortField;
  sortOrder: SortOrder;
  /** Whether to render "Advanced filters" already expanded -- used so a shared/reloaded
   * URL with an active advanced filter doesn't hide the reason for its own results. */
  advancedFiltersActive: boolean;
  onSubmit: (values: FilterFormValues, sortBy: SortField, sortOrder: SortOrder) => void;
  onClear: () => void;
}

/** The free-text name search is the primary, always-visible action; every other filter
 * and the sort controls live inside a native <details> "Advanced filters" disclosure --
 * collapsible without any custom JS-driven show/hide, so it stays keyboard-operable and
 * accessible for free. Everything is one <form>, applied together on a single submit --
 * not a request per keystroke. `first_name`/`last_name` are deliberately not surfaced:
 * FBI (the only source persisted today) never populates those name-component fields, so
 * the controls would always silently match nothing. See README. */
  ...
  return (
    <form className="search-filters" onSubmit={handleSubmit} role="search" aria-label="Case search filters">
      <div className="primary-search">
        ...
        <div className="primary-search-actions">
        ...
        <div className="advanced-filters-grid">
          ...
          <div className="filter-field">
            ...
            <select
              ...
              onChange={(e) => setDraftSortOrder(e.target.value as SortOrder)}
            >
