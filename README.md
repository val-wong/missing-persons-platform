...
apps/web/      Frontend (React + TypeScript + Vite) -- see "Frontend" below
```
...
`apps/web` is a minimal React + TypeScript + Vite frontend for the case search API. It
covers a case search/list page and a case detail page — no authentication, admin UI,
maps, saved searches, or NCMEC integration. All examples below use synthetic data.

### Running it locally

This machine may not have Node installed, so the frontend runs via Docker Compose
alongside the backend, the same way the API does:

```bash
docker compose up -d db api web
```

This serves the frontend at `http://localhost:5173`, talking to the API at
`http://localhost:8000/api/v1` (the default `VITE_API_BASE_URL`). If you do have Node
20+ installed locally, you can instead run it directly:
...
|---|---|---|
| `VITE_API_BASE_URL` | Base URL of the backend API. Not a secret — this is a plain URL, and everything Vite bundles under a `VITE_` prefix is publicly readable in the shipped JavaScript. | `http://localhost:8000/api/v1` |

Copy `apps/web/.env.example` to `apps/web/.env.local` to override it locally.

### Screens

- **Case search** (`/`) — free-text search (`q`), filters (`sex`, `missing_state`,
  `missing_city`, `missing_country`, `hair_color`, `eye_color`, `source`,
  `missing_date_from`/`missing_date_to`), sorting (`sort_by`/`sort_order`), and
  limit/offset pagination (25 per page). Search/filter/sort/page state is kept in the
  URL's query string (e.g. `/?missing_state=CO&sort_by=name&sort_order=asc&offset=25`),
  so a search is shareable and survives a reload. `first_name`/`last_name` are
  deliberately not exposed as filter controls yet: FBI, the only source persisted
  today, never populates those name-component fields (see
  [docs/fbi-normalization.md](docs/fbi-normalization.md)), so the controls would always
  silently match nothing.
- **Case detail** (`/cases/:caseId`) — person fields (including nested height/weight
  objects and photos), case fields, and a provenance section per contributing source
  (code/name, source record ID, source URL as an external link, first/last seen,
  source-modified timestamp, and contributed fields) — never the raw `SourceSnapshot`
  payload.

### A note on empty results

Because FBI (the only source ingested so far) doesn't populate every canonical field,
some filters will legitimately return no matches today — for example `missing_state`,
`missing_city`, `missing_country`, and the missing-date range, since FBI's Wanted API
doesn't report a structured disappearance location or date. This is expected, not a
bug; see [docs/fbi-normalization.md](docs/fbi-normalization.md) for exactly which
fields FBI does and doesn't provide.

### Frontend tests

```bash
docker compose run --rm web sh -c "npm install && npm test"
docker compose run --rm web sh -c "npm install && npm run build"
```

Tests use Vitest + React Testing Library, with the API client module mocked — no real
